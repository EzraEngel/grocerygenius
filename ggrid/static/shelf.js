...
document.addEventListener('keydown', function(e) {
  ...
  if (currentlyHighlighted && currentlyHighlighted.classList.contains('card')) {
    ...
    switch (e.key) {
      case 'ArrowUp':
        newId = currentId - 4; // Assuming 4 cards per row
        break;
      case 'ArrowDown':
        newId = currentId + 4;
        break;
      case 'ArrowLeft':
        newId = currentId - 1;
        if (currentId % 4 === 1) return; // Prevents moving left from the first card in a row
        break;
      case 'ArrowRight':
        newId = currentId + 1;
        if (currentId % 4 === 0) return; // Prevents moving right from the last card in a row
        break;
      default:
