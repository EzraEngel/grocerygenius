...
document.addEventListener('keydown', function(e) {
  if (!modal_displayed()) {
    ...
    switch (e.key) {
        case 'ArrowUp':
          currentlyHighlighted.previousElementSibling.classList.add('highlight');
          currentlyHighlighted.classList.remove('highlight');
          break;
        case 'ArrowDown':
          currentlyHighlighted.nextElementSibling.classList.add('highlight');
          currentlyHighlighted.classList.remove('highlight');
          break;
        default:
          return; // Exit if it's not an arrow key
...
//     // Code to rehighlight the first card or other actions
//     highlightCard(1);
//   });
// });
...
document.querySelectorAll('.ingredient-row').forEach(function (ingredientRow) {
  ingredientRow.addEventListener("click", function (event) {
    ...
    let cartAmount = ingredientRow.querySelector('.cart_amount')
    cartAmount.innerText += 1;
